  ...
  (let* ((project (expand-file-name "sources" jb-kotlin-smoke-root))
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (let ((renamed nil) (idle-timer (symbol-function 'run-with-idle-timer)) idle-callback)
            (cl-letf (((symbol-function 'run-with-idle-timer)
              ...
              (jb-refactor-wait (lambda () (and jb-refactor-done idle-callback))
                                "queued idle editor command")
              ;; Batch Emacs has no idle command loop.
              (apply (car idle-callback) (cdr idle-callback))
              (jb-refactor-wait (lambda () renamed)
